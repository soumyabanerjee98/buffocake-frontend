...
import { smtp_cred } from "../../config/secret";

type SendOfflineMailProps = {
  orderId: string;
  items: any[];
  total: string;
  address: any;
...
const SendOfflineMail = (props: SendOfflineMailProps) => {
  const { orderId, items, total, address } = props;
  ...
  const SendMail = () => {
    ...
    if (element?.innerHTML !== "" && element?.innerText !== "") {
      html2canvas(element).then((canv: any) => {
        ...
        let config = {
          SecureToken: smtp_cred?.security_token,
          To: mail,
          From: smtp_cred?.from,
  ...
  return (
    <>
      ...
      <div className="modal">
        <div className="offline-mail-send">
            ...
            <table style={{ marginTop: "2rem", border: "1px solid black" }}>
              ...
              <tr>
                <td>
                  ...
                  <div>{`${address?.receiverContact}`}</div>
                </td>
              </tr>
            </table>
          </div>
          <div className="confirmation">
            <div className="label">Send this receipt via email?</div>
            <input
              className="email"
              value={mail}
